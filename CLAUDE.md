...
2. **Native `ZGTerminal` class** (`src/zgterminal.cpp/.h`) — a `Control` that is a small terminal emulator:
   ...
   - **Rendering** (`NOTIFICATION_DRAW`): background, then per visible row (via the virtual mapping) per-cell bg rect + search highlight + selection tint + glyph (`draw_char`). The default font prefers a Nerd Font (`kFontCandidates`) so box-drawing/icons render. Cursor shape honors DECSCUSR (block/underline/bar), drawn only at the live bottom (`scroll_offset == 0`).
