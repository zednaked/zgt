...
Done: truecolor, scrollback+wheel, selection/copy, bracketed paste, fish-compat queries, shell starts in the project root, OSC window-title in the panel header, per-child reaping (no global `SIGCHLD`), **mouse tracking** (`?1000/1002/1003` + SGR `?1006`, Shift = local selection), **focus reporting** (`?1004`), and **font zoom** (`Ctrl+Shift+=/-/0`). Not yet: scrollback search, cursor-shape (DECSCUSR), OSC 52 clipboard, persisted font-size/theme config, `sixel`/image protocols, OSC 8 hyperlinks, reflow of scrollback on resize (only the live grid reflows).
